...
把 AgenticCLM 合同全生命周期管理系统打包为 Claude Code / NoBuddy 插件：通过 MCP 暴露全部业务能力（当前生产环境 111 个工具，覆盖合同、审批、履约、合规、系统 5 个域），并附 5 个领域 agent 与 6 个工作流 skill，让客户端按任务自动匹配，而不是面对一堆工具临场摸索。
