...
把 AgenticCLM 合同全生命周期管理系统打包为 Claude Code / NoBuddy 插件：通过 MCP 暴露全部业务能力（当前生产环境 113 个工具，覆盖合同、审批、履约、合规、系统 5 个域），并附 5 个领域 agent 与 6 个工作流 skill，让客户端按任务自动匹配，而不是面对一堆工具临场摸索。
...
- **权限一致**：持有角色管理权限的账号可管理本公司角色，但只能在自身权限点和数据范围内继续缩小，不能越权授予。
- **角色解析级别**：新角色自动归属当前公司并默认为本公司级；只有用于下级向上级上报审批时才使用跨公司级。
- **写操作确认**：创建/修改/删除/提审等写操作仍由宿主审批确认，系统侧同时落 AuditLog 审计。
- **公司边界**：当前公司是普通写操作的硬边界；上级可按权限查看下级业务，但不能直接修改下级合同、部门、用户、相对方、履约任务或电子签请求。
- **逐级管理员委派**：公司管理员可通过专用工具为自己的严格下级公司创建管理员；新管理员固定为目标公司 `ENTITY` 数据范围，可继续向更下一级委派，但不得跨分支或向上授权。
- **委派管理员生命周期**：上级公司可在严格下级范围内重置委派管理员密码、启用、停用或软删除；停用/删除使旧会话失效并自动转交未完成审批待办。
